...
export const APITester: React.FC<APITesterProps> = ({
  ...
  const endpoints = useMemo(() => {
    const bank: EndpointPreset[] = [
      { value: '/api/bank/balance', method: 'GET', body: '{\n  "userId": ""\n}' },
      {
        value: '/api/bank/transfer',
        method: 'POST',
        body: '{\n  "fromUserId": "",\n  "toUserId": "",\n  "amount": 0\n}',
      },
      { value: '/api/bank/deposit', method: 'POST', body: '{\n  "userId": "",\n  "amount": 0\n}' },
      { value: '/api/bank/withdraw', method: 'POST', body: '{\n  "userId": "",\n  "amount": 0\n}' },
      { value: '/api/bank/transactions', method: 'GET', body: '{\n  "userId": ""\n}' },
    ];
    const crypto: EndpointPreset[] = [
      { value: '/api/crypto/balance', method: 'GET', body: '{\n  "userId": ""\n}' },
      {
        value: '/api/crypto/transfer',
        method: 'POST',
        body:
      ...
      { value: '/api/crypto/transactions', method: 'GET', body: '{\n  "userId": ""\n}' },
    ];
    return { bank, crypto };
  }, []);
  ...
  const currentPreset = useMemo(
    () => allPresets.find((e) => e.value === endpoint),
    [allPresets, endpoint]
  );
  ...
  const handleSendRequest = async () => {
    ...
    try {
      ...
      if (endpoint === '/api/bank/balance') {
        ...
        response = await apiSimulator.withdraw(apiKey, body.userId, body.amount);
      } else if (endpoint === '/api/bank/transactions') {
        response = await apiSimulator.getTransactionHistory(apiKey, body.userId);
      } else if (endpoint === '/api/crypto/balance') {
        response = await apiSimulator.getCryptoBalance(apiKey, body.userId);
      } else if (endpoint === '/api/crypto/transfer') {
        response = await apiSimulator.cryptoTransfer(
  ...
  const clearHistory = () => {
    // We can't mutate parent state here; offer a minimal UX:
    // Add a "system" entry indicating a clear request, or the host page can supply a handler.
    // If you want a real clear, wire a prop like `onClearRequests`. For now, noop.
    // (Keeping constraint: "don’t touch other files".)
    ...
    <>
      ...
      {showTester && (
        <div className="fixed right-8 bottom-24 z-40 flex max-h-[80vh] w-full max-w-3xl flex-col overflow-hidden rounded-xl border border-neutral-700 bg-neutral-800 shadow-[0_8px_24px_rgba(15,23,42,0.16)]">
          <div className="flex items-center justify-between border-b border-neutral-700 bg-neutral-900 p-4">
            <h3 className="text-lg font-bold text-neutral-100">API Request Tester</h3>
            <div className="flex items-center gap-2">
              <button
                onClick={fillSample}
          ...
          <div className="flex-1 space-y-4 overflow-y-auto p-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="col-span-1">
                <label className="mb-2 block text-sm font-medium text-neutral-300">Method</label>
                <select
                  value={method}
              ...
              <div className="col-span-2">
                <label className="mb-2 block text-sm font-medium text-neutral-300">Endpoint</label>
                <select
                  value={endpoint}
                  onChange={(e) => {
                    const selected = allPresets.find((ep) => ep.value === e.target.value);
                    if (selected) {
                      setEndpoint(selected.value);
                      setMethod(selected.method);
                      setRequestBody(selected.body);
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                <label className="block text-sm font-medium text-neutral-300">Request Body (JSON)</label>
                <div className="flex items-center gap-2">
                  <button
                    onClick={prettify}
              ...
              <textarea
                ...
                className={`bg-code-bg text-code-text w-full rounded-lg border px-4 py-2 font-mono text-sm focus:outline-none ${
                  jsonError ? 'border-accent-error focus:border-accent-error' : 'border-neutral-600 focus:border-primary-600'
                }`}
                placeholder='{"userId": "abc123"}'
              />
              {jsonError && (
                <p className="mt-1 text-xs text-accent-error">JSON error: {jsonError}</p>
              )}
            </div>
            ...
            {requests.length > 0 && (
              <div>
                <div className="mb-3 flex items-center justify-between">
                  <h4 className="text-sm font-bold text-neutral-300">Recent Requests</h4>
                  <button
                ...
                <div className="max-h-72 space-y-2 overflow-y-auto">
                  {requests.slice(0, 6).map((req) => (
                    <div key={req.id} className="rounded-lg border border-neutral-600 bg-neutral-700 p-3">
                      <div className="mb-2 flex items-center justify-between">
                        <div className="flex flex-wrap items-center gap-2">
                          <span
                            className={`rounded px-2 py-1 text-xs font-bold ${
                              req.method === 'GET' ? 'bg-accent-info text-white' : 'bg-accent-success text-white'
                            }`}
                          >
                            {req.method}
                          </span>
                          <span className="font-mono text-xs text-neutral-300">{req.endpoint}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span
                            className={`rounded px-2 py-1 text-xs font-bold ${
                              req.status === 200 ? 'bg-accent-success text-white' : 'bg-accent-error text-white'
                            }`}
                          >
                            {req.status}
                          </span>
                          {typeof (req.response as any)?._latencyMs === 'number' && (
                      ...
                      <div className="bg-code-bg mb-2 rounded p-2">
                        <div className="mb-1 flex items-center justify-end gap-2">
                          <button
                            onClick={() => copy(JSON.stringify(req.response, null, 2))}
                      ...
                      <p className="text-xs text-neutral-500">{formatDate(req.timestamp)}</p>
                    </div>
                  ))}
                </div>
              </div>
